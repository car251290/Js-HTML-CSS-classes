...
// get the variables set for the selection to show what are they doing.
...
// the times a player can guess the answer 
// Make a variable with the time
let maxGuesses = 6;
function resetGame (){
...
function gameOver (isVictory) {
    ...
    <b>${currentWord}</b>`;
    // the gameModel it is a classList the add to show
    gameModal.classList.add("show");
}
// function to click in the hangman to add a letter.
function initGame (button, clickedLetter) {
    // Checking if clickedLetter is exist on the currentWord
    if(currentWord.includes(clickedLetter)) {
        // Showing all correct letters on the word display
        [...currentWord].forEach((letter, index) => {
            // the letter is to clicke and check. 
